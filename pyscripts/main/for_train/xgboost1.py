...
# 平均时间
travel_time_train = travel_time_train_data

# 流量
volume_train = volume_train_data
...
time_model = xgb.train(plst, time_train, num_rounds, feval=MAPE)
...
params = {
    ...
    'max_depth':6,          #树的深度越大，则对数据的拟合程度越高（过拟合程度也越高）。即该参数也是控制过拟合,建议通过交叉验证（xgb.cv ) 进行调参,通常取值：3-10
...
volume_model = xgb.train(plst, volume_train, num_rounds, feval=MAPE)
